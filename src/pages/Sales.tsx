...
export default function Sales() {
  ...
  const handleProcessSale = async () => {
    ...
    // CRITICAL FIX: More robust user validation
    if (!state.user) {
    ...
    // CRITICAL FIX: Validate user has required fields with better error messages
    if (!state.user.id) {
    ...
    try {
      ...
      const saleData = {
        ...
        totalUSD,
        totalVES,
        userId: state.user.id, // Now safely accessed after validation
        userName: state.user.name, // Now safely accessed after validation
        ...(paymentMethod === 'mixed' && {
  ...
  const handleCashClosure = async () => {
    ...
    try {
      // Save closure data to localStorage for history
      ...
      // Clear sales if using clearSales function
      if (clearSales) {
        await clearSales();
      }
